...
<div class="w-full px-3 my-4 sm:px-6 h-fit" x-data="{banner:false,search:false}">
    ...
    <div class="fixed inset-0 z-20 flex items-center justify-center w-full h-full overflow-auto bg-black/80" x-show="banner" x-transition>
        ...
        <img src="{{ asset('images/banner-stage.jpeg') }}" alt="Banner Stage" class="relative w-full px-4 py-8 sm:py-4 sm:h-full h-fit aspect-auto">
    </div>
    <div class="px-2 py-4 my-4 border-2 rounded-lg shadow-lg sm:px-6 sm:py-8 border-sekunder">
        ...
        <form action="{{ route('the_stage.store') }}" method="post" x-data="{modal:false}">
            ...
            <div class="grid grid-cols-3 gap-6 mt-4 md:grid-cols-3">
                ...
                <span class="col-span-3 px-4 py-2 font-bold text-center text-white uppercase transition-all border-2 rounded cursor-pointer bg-sekunder hover:text-sekunder border-sekunder hover:bg-white" x-on:click="modal=!modal">Daftar</sp>
            </div>

            <div class="fixed inset-0 z-20 flex items-center justify-center w-full h-full overflow-auto bg-black/80" style="display:none;" id="modal" x-show="modal" x-transition>
                <i class="absolute z-10 text-3xl text-white transition cursor-pointer fa-solid fa-xmark top-5 right-10 hover:text-premier" x-on:click="modal=false"></i>
                <div class="flex flex-col w-full p-4 mx-4 overflow-hidden bg-white rounded-lg sm:w-1/2">
                    ...
                    <button type="submit" class="w-full col-span-3 px-4 py-2 mt-6 font-bold text-white uppercase transition-all border-2 rounded bg-sekunder hover:text-sekunder border-sekunder hover:bg-white ">Daftar</button>
                </div>
            </div>
        </form>
    </div>
