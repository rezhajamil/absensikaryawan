...
<div class="w-full px-3 my-4 sm:px-6 h-fit" x-data="{banner:false,search:false}">
    <div class="w-full overflow-hidden rounded-lg h-fit sm:h-[500px] object-center cursor-pointer relative group" x-on:click="banner=!banner">
        ...
        <img src="{{ asset('images/banner-esport.jpg') }}" alt="Banner Ambassador" class="object-contain w-full">
    </div>
    <div class="fixed inset-0 z-20 flex items-center justify-center w-full h-full overflow-auto bg-black/80" x-show="banner" x-transition>
        ...
        <img src="{{ asset('images/banner-esport.jpg') }}" alt="Banner Ambassador" class="relative w-full px-4 py-8 sm:py-4 sm:h-full h-fit aspect-auto">
    </div>
    <div class="px-2 py-4 my-4 border-2 rounded-lg shadow-lg sm:px-6 sm:py-8 border-sekunder">
        ...
        <form action="{{ route('esport.store') }}" method="post">
            @csrf
            <div class="grid grid-cols-4 gap-6 mt-4 md:grid-cols-4">
                ...
                <button type="submit" class="px-4 py-2 font-bold text-white uppercase transition-all border-2 rounded col-span-full bg-sekunder hover:text-sekunder border-sekunder hover:bg-white ">Daftar</button>
            </div>
        </form>
    </div>
